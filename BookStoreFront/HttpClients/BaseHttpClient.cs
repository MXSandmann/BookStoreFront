...
namespace BookStoreFront.HttpClients
{
    public abstract class BaseHttpClient
    {
        ...
        protected virtual async Task<APIResponse<T>> GetAsync<T>(string url)
        {
            var response = await client.GetAsync(url);
            return JsonConvert.DeserializeObject<APIResponse<T>>(await response.Content.ReadAsStringAsync());
        }

        protected virtual async Task<APIResponse<T>> PostAsync<T>(string url, object body)
        {
        ...
        protected virtual async Task<ApiResponse> PostAsync(string url, object body)
        {
            var content = new StringContent(JsonConvert.SerializeObject(body), Encoding.UTF8, "application/json");
            var response = await client.PostAsync(url, content);
